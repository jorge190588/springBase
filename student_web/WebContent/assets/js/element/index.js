...
var Module =  function(){
	...
	_private.fillDropdown=function(form,callback){
		_private.apiFront.get('entiti/findall',null,function(response,error){
			form.entiti.fill(response._data,'id','name');
			_private.apiFront.get('elementType/findall',null,function(response,error){
				form.elementType.fill(response._data,'id','name');
				callback();	
			});
		});
	};
